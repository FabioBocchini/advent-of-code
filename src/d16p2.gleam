...
fn solve(contraption: Contraption) -> Int {
  let height = list.length(contraption)
  let width = {
    let assert [hd, ..] = contraption
    list.length(hd)
  }

  let max_d =
    list.fold(list.range(0, width - 1), 0, fn(acc, i) {
      walk(contraption, [#(0, i, D)], dict.new(), dict.new(), 0)
      |> int.max(acc)
    })

  let max_u =
    list.fold(list.range(0, width - 1), 0, fn(acc, i) {
      walk(contraption, [#(height - 1, i, U)], dict.new(), dict.new(), 0)
      |> int.max(acc)
    })

  let max_r =
    list.fold(list.range(0, height - 1), 0, fn(acc, j) {
      walk(contraption, [#(j, 0, R)], dict.new(), dict.new(), 0)
      |> int.max(acc)
    })

  let max_l =
    list.fold(list.range(0, height - 1), 0, fn(acc, j) {
      walk(contraption, [#(j, width - 1, L)], dict.new(), dict.new(), 0)
      |> int.max(acc)
    })
