...
fn count_deletable(bricks: List(Brick), count: Int) -> Int {
  case bricks {
    [] -> count
    [hd, ..tl] ->
      case
        list.fold_until(tl, True, fn(_, b) {
          case b.held_by == [hd.id] {
            True -> Stop(False)
            False -> Continue(True)
          }
        })
      {
        True -> count_deletable(tl, count + 1)
        False -> count_deletable(tl, count)
      }
  }
}
